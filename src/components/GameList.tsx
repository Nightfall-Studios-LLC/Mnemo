  ...
  return <section className="game-panel">
    ...
    <div className="game-rows">{games.map(game => {
      ...
      return <button key={game.id} className={`game-row ${selected === game.id ? "selected" : ""}`} onClick={() => onSelect(game.id)}>
        <span className="game-cell"><GameArtwork gameId={game.id} name={game.name} steamAppId={game.steamAppId} artwork={game.artwork}/><span><strong>{game.name}</strong><small>{game.launcher}</small></span></span>
