...
export default function App() {
  const [page, setPage] = useState<Page>(readInitialPage); const [games, setGames] = useState<Game[]>(demoGames); const [selectedId, setSelectedId] = useState(demoGames[0].id);
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);
  const [animations, setAnimations] = useState(() => localStorage.getItem("mnemo-animations") !== "off");
  ...
  useEffect(() => {
    Promise.allSettled([
      invoke<{ providers?: BackupProvider[] }>("load_configuration"),
  ...
  useEffect(() => { localStorage.setItem("mnemo-animations", animations ? "on" : "off"); }, [animations]);
  ...
  const backup = async (all = false) => { const targets = (all ? games.flatMap(g => g.saveTargets.map(t => [g,t] as const)) : selected.saveTargets.map(t => [selected,t] as const)).filter(([,t]) => t.enabled); notify(`Backing up ${targets.length} save location${targets.length === 1 ? "" : "s"}…`); let completed = 0; for (const [g,t] of targets) { try { const p = providers.find(p => p.enabled)?.destination || ""; const snap = await invoke<BackupSnapshot>("create_backup", { gameId: g.id, target: t, destination: p }); setSnapshots(s => [snap, ...s]); completed++; } catch { /* demo paths may not exist */ } } notify(completed ? `${completed} backup${completed === 1 ? "" : "s"} completed` : "Choose a real save folder and storage location to start backing up"); };
  const beginRestore = (s?: BackupSnapshot) => { setRestoreSnapshot(s || snapshots.find(x => x.gameId === selected.id)); setDialog("restore"); };
  const content = page === "Library" ? <>
    <Toolbar search={search} onSearch={setSearch} filter={filter} onFilter={() => setFilter(value => !value)} sort={sort} onSort={setSort} onBackupAll={() => backup(true)}/>
    <div className="library"><GameList games={visible} selected={selectedId} onSelect={setSelectedId}/>{selected && <Inspector key={selected.id} game={selected} snapshots={snapshots} onToggle={toggle} onAdd={() => setDialog("target")} onBackup={() => backup()} onRestore={beginRestore} notify={notify}/>}</div>
  </> : page === "Backups" ? <BackupsPage snapshots={snapshots} games={games} onRestore={beginRestore}/>
    : page === "Providers" ? <ProvidersPage providers={providers} snapshots={snapshots} onAdd={() => setDialog("provider")} notify={notify}/>
      : page === "Activity" ? <ActivityPage snapshots={snapshots} games={games}/>
        : <SettingsPage animations={animations} onAnimationsChange={setAnimations}/>;
  return <main className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""} ${animations ? "" : "animations-off"}`}>
    <Sidebar page={page} collapsed={sidebarCollapsed} onChange={setPage} onCollapse={() => setSidebarCollapsed(value => !value)}/>
    <div className="workspace">{content}</div>
    {dialog === "target" && <PathDialog
      title="Add save location"
      confirmLabel="Add save"
      onClose={() => setDialog(null)}
      onConfirm={(path,name) => { const id = crypto.randomUUID(); setGames(gs => gs.map(g => g.id !== selected.id ? g : ({...g, saveTargets: [...g.saveTargets, { id, gameId: g.id, name: name || "Custom Save", sourcePath: path, enabled: true, providerIds: ["local"], status: "warning" }]}))); setDialog(null); notify("Save location added"); }}
    />}
    {dialog === "provider" && (
      <PathDialog
        title="Add storage folder"
        confirmLabel="Add folder"
        onClose={() => setDialog(null)}
        onConfirm={(path, name) => {
          setProviders(current => [...current, {
            id: crypto.randomUUID(),
            providerType: path.startsWith("\\\\") ? "nas" : "local",
...
function readInitialPage(): Page {
  ...
  return (["Library", "Backups", "Providers", "Activity", "Settings"] as Page[]).includes(value as Page) ? value as Page : "Library";
}
